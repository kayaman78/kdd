...
#   2. .env file in Docker Compose project directory (via container labels)
#      Works for both bind mounts AND named volumes, since credentials
...
get_env_from_file() {
    local file="$1"
    local var="$2"
    grep -E "^${var}=" "$file" 2>/dev/null | head -1 | cut -d= -f2- | tr -d '"' | tr -d "'"
}

# Resolve a credential for a container:
#   1. Direct environment variable
#   2. .env file in the Compose project working directory (label-based)
#
# Works transparently for both bind-mount and named-volume stacks,
# since credentials live in the compose folder, not inside the volume.
get_container_env() {
    local container="$1"
    local var="$2"
    local val=""
    ...
    # --- 2. .env in Compose project directory (via label) ---
    local compose_dir
    compose_dir=$(docker inspect "$container" 2>/dev/null | \
        jq -r '.[0].Config.Labels["com.docker.compose.project.working_dir"] // empty' \
        2>/dev/null || true)

    if [ -n "$compose_dir" ] && [ -f "${compose_dir}/.env" ]; then
        val=$(get_env_from_file "${compose_dir}/.env" "$var")
        if [ -n "$val" ]; then
            log "  [.env: ${compose_dir}]"
            echo "$val"
            return 0
...
add_entry() {
    ...
    local exists
    exists=$(yq e ".${section}[] | select(.name == \"$name\") | .name" "$CONFIG" 2>/dev/null || true)
    if [ -n "$exists" ]; then
        log "  Already in config: $name"
        return 0
    fi

    # Append entry with just the name first
    yq eval ".${section} += [{\"name\": \"${name}\"}]" -i "$CONFIG"
    local idx
    idx=$(yq eval ".${section} | length - 1" "$CONFIG")

    # Add each key=value field
    while [ $# -gt 0 ]; do
        local key="${1%%=*}"
        local value="${1#*=}"
...
while IFS= read -r container; do
    ...
    image=$(docker inspect "$container" 2>/dev/null | jq -r '.[0].Config.Image' || true)
    [ -z "$image" ] && continue

    # Extract stack name from container name (e.g., nextcloud_db_1 -> nextcloud)
    ...
    if echo "$image" | grep -qiE 'mysql|mariadb'; then
        ...
        # Prefer root, fall back to named user
        if [ -n "$root_pass" ]; then
            user="root"
            pass="$root_pass"
